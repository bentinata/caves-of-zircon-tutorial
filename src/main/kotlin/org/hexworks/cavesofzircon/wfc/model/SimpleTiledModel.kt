package org.hexworks.cavesofzircon.wfc.model

import com.google.gson.Gson
import org.hexworks.cavesofzircon.wfc.model.data.SampleData
import org.hexworks.cavesofzircon.wfc.trimToColor
...
class SimpleTiledModel(width: Int, height: Int, val name: String, subsetName: String, periodic: Boolean, black: Boolean) : Model(width, height) {
    ...
    override fun coordinates(): Array<Array<String>> {
        var result = Array(FMX) { Array (FMY) { "" } }
                ...
                for (y in 0 until FMY) {
                    ...
                    for (yt in 0 until tilesize)
                        for (xt in 0 until tilesize) {
                            ...
                            result[x][y] = Integer.toHexString(c!!.rgb)
                        }
                }
        } else {
            for (x in 0 until FMX) {
                    ...
                    for (yt in 0 until tilesize) {
                        for (xt in 0 until tilesize) {
                            if (black && amount == tCounter) {
                                ...
                                result[x][y] = Integer.toHexString(blackColor.rgb)
                            } else {
                                var r = 0.0
                                var g = 0.0
                                var b = 0.0
                                ...
                                result[xCord][yCord] = Integer.toHexString(color.rgb)
                            }
                        }
                    }
                }
