...
class OverlappingModel(private val bitmap: BufferedImage, val N: Int, width: Int, height: Int, periodicInput: Boolean,
    ...
    override fun coordinates(): Array<Array<String>> {
        val result = Array(FMX) { Array(FMY) { "" } }
        if (observed != null) {
            for (y in 0 until FMY) {
                ...
                for (x in 0 until FMX) {
                    ...
                    result[x][y] = Integer.toHexString(temp)
                }
            }
        } else {
            for (i in 0 until wave.size) {
                ...
                result[x][y] = Integer.toHexString(color.rgb)
            }
        }
